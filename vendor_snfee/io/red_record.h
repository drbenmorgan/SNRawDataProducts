...
namespace snfee {
  namespace io {
    ...
    public:
      ...
      void install_rtd(const std::shared_ptr<const snfee::data::raw_trigger_data> & rtd_ptr_,
                       const snfee::data::timestamp & timeshift_);
    
      void print(std::ostream & out_) const;

      friend std::ostream & operator<<(std::ostream & out_, const red_record & rec_);
   
    private:

      std::shared_ptr<snfee::data::raw_event_data> _red_; ///< Pointer to a raw event data (RED)
