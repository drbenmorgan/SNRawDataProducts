...
namespace snfee {
  namespace io {
    ...
    int raw_record_flow_statistics::inversion_record::get_dist() const
    {
      return pos2 - pos1;
    }
  
    raw_record_flow_statistics::raw_record_flow_statistics(const size_t buffer_size_,
                                                           const bool debug_)
    {
      if (debug_) set_logging(datatools::logger::PRIO_DEBUG);
      _buffer_size_ = buffer_size_;
      _init_();
      return;
    }

    datatools::logger::priority raw_record_flow_statistics::get_logging() const
    {
      return _logging_;
    }

    void raw_record_flow_statistics::set_logging(const datatools::logger::priority p_)
    {
      _logging_ = p_;
      return;
    }

    int raw_record_flow_statistics::get_min_trigger_id()
    {
      return _first_positions_.begin()->first;
    }

    int raw_record_flow_statistics::get_max_inversion_distance()
    {
      return _max_inversion_distance_;
    }
   
    void raw_record_flow_statistics::_init_()
    {
    ...
    void raw_record_flow_statistics::add(const int pos_, const int tid_)
    {
      ...
      for (auto p : _first_positions_) {
        if (tid_ < p.first) {
          ...
          int pos1 = p.second;
          int dist = pos_ - pos1;
          DT_LOG_DEBUG(_logging_, "Detected inversion : trigger ID=" << tid_ << "@" << pos_
                       << " < " << tid1 << "@" << pos1);
          _inversions_[tid_] = inversion_record(pos1, pos_, tid1, tid_);
          _hdistances_.add(dist);
          if ((_max_inversion_distance_ < 0) || (dist > _max_inversion_distance_)) {
