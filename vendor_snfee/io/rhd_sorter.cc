...
namespace snfee {
  namespace io {
    ...
    void rhd_sorter::set_config(const config_type & cfg_)
    {
      DT_THROW_IF(is_initialized(), std::logic_error,
                  "Sorter is already initialized!");
      _config_ = cfg_;
      return;
    }

    bool rhd_sorter::is_initialized() const
    {
      return _initialized_;  
    }
        
    void rhd_sorter::initialize()
    {
      DT_THROW_IF(is_initialized(), std::logic_error,
                  "Sorter is already initialized!");
      ...
      if (_pimpl_->mode == MODE_EVALUATION) {
        _pimpl_->no_store = true;
        _pimpl_->stats.reset(new raw_record_flow_statistics(_config_.evaluation_buffer_size));
      ...
      for (int ifile = 0; ifile < (int) _config_.input_rhd_filenames.size(); ifile++) {
      ...
      DT_LOG_DEBUG(_logging_, "no_store = " << std::boolalpha << _config_.no_store);
      ...
      if (with_writer) {
        ...
        if (!_config_.output_rhd_listname.empty()) {
          ...
          datatools::fetch_path_with_env(listname);
          std::ifstream finput_list(listname.c_str());
          DT_THROW_IF(!finput_list, std::logic_error, "Cannot open output files list filename!");
          while (finput_list and ! finput_list.eof()) {
            std::string line;
            std::getline(finput_list, line);
            boost::trim(line);
        ...
        for (int ifile = 0; ifile < (int) _config_.output_rhd_filenames.size(); ifile++) {
    ...
    void rhd_sorter::_output_sorted_record_(const snfee::data::raw_record_ptr & ph_)
    {
      // Statistics:
      ...
      if (_pimpl_->max_output_trigger_id < 0) {
        ...
        if (trigger_id_less(tid, _pimpl_->max_output_trigger_id)) {
           DT_THROW(std::logic_error, "Output a record #" << _pimpl_->output_hit_counter
                    << " with trigger ID=[" << tid << "]"
      ...
      if (_pimpl_->writer) {
        if (ph_) {
          ...
          DT_THROW(std::logic_error, "No raw record to be stored! This is a bug!");
        }
        ...
        } else if (typeid(*ph_.get()) == typeid(snfee::data::tracker_hit_record)) {
    ...
    void rhd_sorter::_finalize_()
    {
      ...
      DT_LOG_DEBUG(_logging_, "About to remove the remaining " << npop << " records from the buffer...");
      for (size_t i = 0; i < npop; i++) {
        snfee::data::raw_record_ptr prec = _pimpl_->cbuffer.front();
        _pimpl_->cbuffer.pop_front();
        _output_sorted_record_(prec);
        DT_LOG_DEBUG(_logging_, "Pop record with trigger ID=[" << prec->get_trigger_id() << "]");
      }
      if (_config_.log_modulo) {
        if ((_pimpl_->output_hit_counter % _config_.log_modulo) == 0) {
          std::clog << "#sorted hits=" << _pimpl_->output_hit_counter << "\n";
    ...
    void rhd_sorter::_input_unsorted_record_(snfee::data::raw_record_ptr & ph_)
    {
      if (ph_) {
        snfee::data::raw_record_interface * rri = ph_.get();
        DT_LOG_DEBUG(_logging_, "Input raw record type = '" << typeid(*rri).name() << "'\n");
      }
      if (_pimpl_->reader->record_tag_is(snfee::data::calo_hit_record::SERIAL_TAG)) {
        if (! ph_ || typeid(*ph_.get()) != typeid(snfee::data::calo_hit_record)) {
      ...
      } else if (_pimpl_->reader->record_tag_is(snfee::data::tracker_hit_record::SERIAL_TAG)) {
        if (! ph_ || typeid(*ph_.get()) != typeid(snfee::data::tracker_hit_record)) {
      ...
      } else if (_pimpl_->reader->record_tag_is(snfee::data::trigger_record::SERIAL_TAG)) {
    ...
    void rhd_sorter::pimpl_type::dump_buffer(std::ostream & out_) const
    {
      ...
      for (auto ph : this->cbuffer) {
        ...
        out_ << "|   "
             << std::setw(8) << pos << ' '
             << std::setw(8) << ph->get_hit_num() << ' '
             << std::setw(8) << ph->get_trigger_id() << ' '
             << std::endl;
        cnt++;
      }
      if (this->cbuffer.size()) {
        out_ << "|-- Front : " << this->cbuffer.front()->get_hit_num() << std::endl;
        out_ << "|-- Back  : " << this->cbuffer.back()->get_hit_num() << std::endl;
      }
      out_ << "|-- Empty : " << std::boolalpha << this->cbuffer.empty() << std::endl;
      out_ << "`-- Full  : " << std::boolalpha << this->cbuffer.full() << std::endl;
      return;
    }
    
    void rhd_sorter::_process_data_(const snfee::data::raw_record_ptr & ph_)
    {
      DT_LOG_DEBUG(_logging_, "Process a new raw record...");
      DT_LOG_DEBUG(_logging_, "Push the new raw record in the buffer with trigger ID=[" << ph_->get_trigger_id() << "]");
      ...
      if (_pimpl_->cbuffer.full()) {
        ...
        DT_LOG_DEBUG(_logging_, "About to remove " << npop << " records from the first half of the buffer...");
        for (size_t i = 0; i < npop; i++) {
          snfee::data::raw_record_ptr prec = _pimpl_->cbuffer.front();
          _pimpl_->cbuffer.pop_front();
          _output_sorted_record_(prec);
        ...
        DT_LOG_DEBUG(_logging_, "New buffer size = " << _pimpl_->cbuffer.size());
        if (datatools::logger::is_debug(_logging_)) {
          _pimpl_->dump_buffer(std::cerr);
        }
      }
    ...
    void rhd_sorter::run()
    {
      ...
      if (_pimpl_->mode == MODE_EVALUATION) {
        if (!_pimpl_->report_filename.empty()) {
          ...
          DT_THROW_IF(!freport, std::logic_error, "Cannot open evaluation report file '" << fn << "'!");
          freport << "#@evaluation_buffer_size=" << _config_.evaluation_buffer_size << std::endl;
          freport << "#@max_inversion_distance=" << _pimpl_->stats->get_max_inversion_distance() << std::endl;
      ...
      if (_pimpl_->mode == MODE_SORT) {
        if (! _pimpl_->report_filename.empty()) {
          ...
          datatools::fetch_path_with_env(fn);
          std::ofstream freport(fn);
          DT_THROW_IF(!freport, std::logic_error, "Cannot open sort report file '" << fn << "'!");
          freport << "#@sort_buffer_size=" << _config_.sort_buffer_size << std::endl;
          freport << "#@input_hit_counter=" << _pimpl_->input_hit_counter << std::endl;
          freport << "#@output_hit_counter=" << _pimpl_->output_hit_counter << std::endl;
          freport << "#@output_trigger_counter=" << _pimpl_->output_trigger_counter << std::endl;
          freport << "#@max_output_trigger_id=" << _pimpl_->max_output_trigger_id << std::endl;
          freport.close();
        }
      }
      return;
