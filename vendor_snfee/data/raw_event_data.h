...
namespace snfee {
  namespace data {
    ...
    public:
      ...
      typedef std::shared_ptr<const snfee::data::raw_trigger_data> const_raw_trigger_data_ptr;

      /// \brief RTD record information:
      struct rtd_record_type
      {
        timestamp time_shift; ///< Time shift with respect to the reference time (typically in unit of 25 ns from main clock F=40 MHz)
      ...
      void fetch_rtd(int index_, timestamp & time_shift_, const_raw_trigger_data_ptr & rtd_ptr_) const;
    ...
    private:
      ...
      timestamp _reference_time_;                   ///< Reference timestamp in the run time frame (typically in unit of 25 ns from main clock F=40 MHz)
      std::vector<rtd_record_type> _rtd_pack_;      ///< Collection of handles for raw trigger data records
...
BOOST_CLASS_EXPORT_KEY2(snfee::data::raw_event_data, "snfee::data::raw_event_data")
