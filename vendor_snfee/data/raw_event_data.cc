...
namespace snfee {
  namespace data {
    ...
    bool raw_event_data::is_complete() const
    {
      if (!has_run_id()) {
        return false;
      }
    ...
    bool raw_event_data::has_reference_time() const
    {
      return _reference_time_.is_valid();
    }

    const timestamp & raw_event_data::get_reference_time() const
    {
      return _reference_time_;
    }

    void raw_event_data::set_reference_time(const timestamp & ref_time_)
    {
      _reference_time_ = ref_time_;
      return;
    }

    void raw_event_data::add_rtd(const timestamp & time_shift_,
                                 const const_raw_trigger_data_ptr & rtd_ptr_)
    {
      DT_THROW_IF(!time_shift_.is_valid(), std::logic_error, "Invalid time shift " << time_shift_ << " for RTD record!");
      if (_rtd_pack_.size()) {
        if (time_shift_ < _rtd_pack_.back().time_shift) {
          DT_THROW(std::logic_error,
                   "Invalid time ordering (" << time_shift_ << "<" << _rtd_pack_.back().time_shift << " of RTD record!");
        }
      }
    ...
    std::size_t raw_event_data::get_number_of_rtd() const
    {
      return _rtd_pack_.size();
    }

    void raw_event_data::fetch_rtd(int index_,
                                   timestamp & time_shift_,
                                   const_raw_trigger_data_ptr & rtd_ptr_) const
    {
      DT_THROW_IF(index_ < 0 or index_ >= (int) _rtd_pack_.size(), std::range_error,
                                    ...
                                    const boost::property_tree::ptree & options_) const
    {
      ...
      out_ << popts.indent << tag
           << "RTD pack : " << _rtd_pack_.size() << std::endl;
      for (std::size_t i = 0; i < _rtd_pack_.size(); i++) {
        ...
        out_ << indent2.str() << tag << "Time shift = " << rtd_record.time_shift << std::endl;
