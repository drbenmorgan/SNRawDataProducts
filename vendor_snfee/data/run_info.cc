...
namespace snfee {
  namespace data {
    ...
    bool run_info::has_run_id() const
    {
      return _run_id_ != INVALID_RUN_ID;
    }

    int32_t run_info::get_run_id() const
    {
      return _run_id_;
    }

    void run_info::set_run_id(const int32_t run_id_)
    {
      _run_id_ = run_id_ < INVALID_RUN_ID ? INVALID_RUN_ID : run_id_;
      return;
    }

    bool run_info::has_run_start_time() const
    {
      return !_run_start_time_.is_not_a_date_time();
    }
    
    void run_info::set_run_start_time(const boost::posix_time::ptime & t_)
    {
      _run_start_time_ = t_;
      return;
    }
    ...
    bool run_info::has_run_stop_time() const
    {
      return !_run_stop_time_.is_not_a_date_time();
    }
    
    void run_info::set_run_stop_time(const boost::posix_time::ptime & t_)
    {
      _run_stop_time_ = t_;
      return;
    }
    ...
    bool run_info::has_run_category() const
    {
      return !_run_category_.empty();
    }
    
    const std::string & run_info::get_run_category() const
    {
      return _run_category_;
    }
    
    void run_info::set_run_category(const std::string & cat_)
    {
      _run_category_ = boost::trim_copy(cat_);
      return;
    }

    bool run_info::has_run_subcategory() const
    {
      return !_run_subcategory_.empty();
    }
    
    const std::string & run_info::get_run_subcategory() const
    {
      return _run_subcategory_;
    }
    
    void run_info::set_run_subcategory(const std::string & cat_)
    {
      _run_subcategory_ = boost::trim_copy(cat_);
      return;
    }

    bool run_info::is_complete() const
    {
      if (!has_run_id()) {
        return false;
      }
    ...
    // virtual
    void run_info::print_tree(std::ostream & out_,
                              const boost::property_tree::ptree & options_) const
    {
      ...
      out_ << popts.indent << tag
           << "Run category     : '" << _run_category_ << "'" << std::endl;

      out_ << popts.indent << tag
           << "Run sub-category : '" << _run_subcategory_ << "'" << std::endl;
