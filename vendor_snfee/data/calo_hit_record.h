  ...
  namespace data {
    ...
    public:
      ...
      private:
        ...
        int32_t _charge_        = 0;     //!< Computed charge (23 signed bits, computed in FEB FPGA)
        int32_t _rising_cell_   = 0;     //!< Positive edge cell index (24 bits, in unit of TDC LSB / 256, computed in FEB FPGA)
        int32_t _falling_cell_  = 0;     //!< Negative edge cell index (24 bits, in unit of TDC LSB / 256, computed in FEB FPGA)
    ...
    private:
      ...
      uint16_t _event_id_   = INVALID_NUMBER_16U; //!< Local event ID (8 bits, should be the trigger ID mod 256)
      uint16_t _l2_id_      = INVALID_NUMBER_16U; //!< L2 signal ID (5 bits, should be the trigger ID mod 32)
      uint16_t _fcr_        = snfee::model::feb_constants::SAMLONG_INVALID_SAMPLE; //!< First cell read (10 bits)
      ...
      uint16_t  _waveform_number_of_samples_ = INVALID_WAVEFORM_NUMBER_OF_SAMPLES; //!< Waveform number of samples (default at construct: 0, typical: 1024)
      ...
      // Trigger rate data:
      uint16_t _lt_trigger_counter_[snfee::model::feb_constants::SAMLONG_NUMBER_OF_CHANNELS]; 
      uint32_t _lt_time_counter_[snfee::model::feb_constants::SAMLONG_NUMBER_OF_CHANNELS]; 
