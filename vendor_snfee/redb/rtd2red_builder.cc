...
namespace snfee {
  namespace redb {
    ...
    rtd2red_builder::rtd2red_builder(builder_pimpl_type & pimpl_,
    ...
    void rtd2red_builder::stop()
    {
      _stop_request_ = true;
      return;
    }

    bool rtd2red_builder::is_stopped() const
    {
      return _stop_request_;
    }

    std::size_t rtd2red_builder::get_red_records_counter() const
    {
      return _red_records_counter_;
    }
    
    int32_t rtd2red_builder::get_next_trigger_id_from_input_buffer() const
    {
    ...
    int32_t rtd2red_builder::get_minimum_trigger_id_from_input_buffer() const
    {
      snfee::data::trigger_id_comparator_type trigger_id_less;
    ...
    void rtd2red_builder::_run_one2one_algo_()
    { 
      DT_LOG_TRACE_ENTERING(_logging_);
      ...
      //         // Extract all RTD records matching the working trigger ID from the input buffer:
      //         while (!ibuf.is_empty()) {
    ...
    void rtd2red_builder::_run_standard_algo_()
    { 
      DT_LOG_TRACE_ENTERING(_logging_);
      // Working RED record:
      snfee::io::red_record red_rec;
      ...
      while (!_stop_request_) {
        ...
        // // Extract the next trigger ID from the input buffer:
        // int32_t fetchable_trig_id = get_minimum_trigger_id_from_input_buffer();
        // bool process_input_rtd = false;
        // if (fetchable_trig_id == snfee::data::INVALID_TRIGGER_ID) {
        ...
        //     DT_LOG_DEBUG(_logging_, "Make the current RTD pushable because input buffers cannot provide RTD records with trigger ID = " << rtd_rec.get_trigger_id() << " anymore.");
        //     push_current_rtd = true;
        //   }
        // } else if (input_buffer_finished) {
        //   DT_LOG_DEBUG(_logging_, "Make the current RTD pushable because this is the end of the input buffer.");
        //   push_current_rtd = true;
        // }
        ...
        //   DT_LOG_NOTICE(_logging_, "Input buffer is finished and there is no more current RTD record. Stop!");
        //   stop();
        // }
        // if ((_rtd_records_counter_ % 100 == 0) or is_stopped()) {
        //   DT_LOG_NOTICE(_logging_, "Builder run : " << _rtd_records_counter_ << " built RED records");  
        // }       
        std::this_thread::yield();
      } // main while loop
