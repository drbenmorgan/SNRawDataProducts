...
namespace snfee {
  namespace redb {
    ...
    void input_worker::stop()
    {
      _stop_request_ = true;
      return;
    }
                        
    bool input_worker::is_stopped() const
    {
    ...
    input_worker::input_worker(const int id_,
                               std::mutex & imtx_,
      ...
      , _buf_(ibuf_)
    {
      ...
      if (!iconfig_.listname.empty()) {
        ...
        DT_THROW_IF(!finput_list, std::logic_error,
    ...
    void input_worker::print(std::ostream & out_) const
    {
    ...
    std::size_t input_worker::get_records_counter() const
    {
      return _records_counter_;
    }
    
    void input_worker::run()
    {
      DT_LOG_TRACE_ENTERING(_logging_);
      _records_counter_ = 0;
      ...
      while (!_stop_request_) {
        ...
        if (rec.empty()) {
          ...
          if (_preader_->has_record_tag()) {
            if (_preader_->record_tag_is(snfee::data::raw_trigger_data::SERIAL_TAG)) {
              DT_LOG_DEBUG(_logging_, "Loading a new raw trigger data record from worker [" << _id_ << "]...");
              rec.make_record();
              _preader_->load(*rec.get_rtd_ptr());
              DT_LOG_DEBUG(_logging_, "New raw trigger data record is loaded from worker [" << _id_ << "]...");
              _records_counter_++;
        ...
        if (!rec.empty()) {
          ...
          if (_buf_.can_push()) {
            ...
            DT_LOG_DEBUG(_logging_, "Inserting the RTD record in the input buffer #" << _id_ << "...");
            _buf_.insert_record(rec);
            DT_LOG_DEBUG(_logging_, "RTD record was inserted in the input buffer #" << _id_ << "...");
            if (datatools::logger::is_debug(_logging_)) {
