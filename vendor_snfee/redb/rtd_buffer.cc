...
namespace snfee {
  namespace redb {
    ...
    rtd_buffer::~rtd_buffer()
    {
      return;
    }

    std::size_t rtd_buffer::size() const
    {
      return _rtd_recs_.size();
    }
     
    std::size_t rtd_buffer::get_capacity() const
    {
      return _capacity_;
    }
       
    /// Reset the RTD buffer
    void rtd_buffer::reset()
    {
      _rtd_recs_.clear();
    ...
    bool rtd_buffer::is_finished() const
    {
      return is_empty() and is_terminated();
    }

    int32_t rtd_buffer::get_front_trig_id() const
    {
      return _front_trig_id_;
    }

    bool rtd_buffer::can_push() const
    {
    ...
    void rtd_buffer::insert_record(const snfee::io::rtd_record rtd_rec_)
    {
      ...
      if (_rtd_recs_.size() == 0) {
        ...
        //// std::deque<snfee::io::rtd_record>::iterator insert_iter = _rtd_recs_.end();
        //// for (std::deque<snfee::io::rtd_record>::iterator it = _rtd_recs_.begin();
        ////      it != _rtd_recs_.end();
        ////      it++) {
    ...
    snfee::io::rtd_record rtd_buffer::pop_record()
    {
      DT_THROW_IF(is_empty(), std::logic_error, "No more record in the RTD buffer!");
    ...
    void rtd_buffer::terminate()
    {
      _terminated_ = true;
      return;
    }

    bool rtd_buffer::is_terminated() const
    {
      return _terminated_;
    }
     
    bool rtd_buffer::is_empty() const
    {
      return _rtd_recs_.size() == 0;
    }

    int32_t rtd_buffer::get_next_poppable_trig_id() const
    {
    ...
    bool rtd_buffer::can_be_popped() const
    {
    ...
    void rtd_buffer::print(std::ostream & out_) const
    {
