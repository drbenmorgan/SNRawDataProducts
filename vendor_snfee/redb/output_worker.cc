...
namespace snfee {
  namespace redb {
    ...
    output_worker::output_worker(std::mutex & omtx_,
                                 red_buffer & obuf_,
    ...
    void output_worker::stop()
    {
      _stop_request_ = true;
      return;
    }

    bool output_worker::is_stopped() const
    {
      return _stop_request_;
    }
    
    std::size_t output_worker::get_records_counter() const
    {
      return _records_counter_;
    }

    std::size_t output_worker::get_stored_records_counter() const
    {
      return _stored_records_counter_;
    }
    
    void output_worker::run()
    {
      ...
      while (!_stop_request_) {
        {
          ...
          while (!_buf_.is_empty()) {
            DT_LOG_DEBUG(_logging_, "Pop RED record from the output RED buffer...");
            _records_counter_++;
        ...
        if (_records_counter_ % 100 == 0 or is_stopped()) {
          DT_LOG_NOTICE(_logging_, "Output worker run : " << _records_counter_ << " saved RED records");  
        }
