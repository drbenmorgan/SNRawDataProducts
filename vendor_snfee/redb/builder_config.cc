...
namespace snfee {
  namespace redb {
    ...
    // static
    std::string builder_config::algo_label(const build_algo_type a_)
    {
    ...
    bool builder_config::has_input_config() const
    {
      return ! input_config.filenames.size();
    }

    const builder_config::input_config_type & builder_config::get_input_config() const
    {
      return input_config;
    }

    void builder_config::set_input_config(const std::string & label_,
                                          const std::vector<std::string> & filepaths_,
                                          const format_type format_)
    {
                                    ...
                                    const boost::property_tree::ptree & options_) const
    {
      ...
      outs << popts.indent << tag
           << "Build algo : " << algo_label(build_algo) << std::endl;

      outs << popts.indent << tag
           << "Input config : " << std::endl;

      outs << popts.indent << skip_tag << tag
           << "Label : '" << input_config.label << "'" << std::endl;

      outs << popts.indent << skip_tag << tag
           << "Filenames : " << input_config.filenames.size() << std::endl;

      for (int ifilename = 0; ifilename < (int) input_config.filenames.size(); ifilename++) {
        ...
        outs << "Filename #" << ifilename << " : '" << input_config.filenames[ifilename] << "'";
        outs << std::endl;
      }

      outs << popts.indent << skip_tag << last_tag
           << "Format : '" << format_label(input_config.format) << "'" << std::endl;
    ...
    void builder_config::check(const builder_config & cfg_)
    {
      DT_THROW_IF(cfg_.run_id == snfee::data::INVALID_RUN_ID,
                  std::logic_error,
                  "Missing run ID!");
      DT_THROW_IF(cfg_.input_config.filenames.size() == 0 and cfg_.input_config.listname.empty(),
                  std::logic_error,
    ...
    void builder_config::load(const std::string & redb_config_filename_,
                              builder_config & cfg_)
    {
      // Clear the config:
      cfg_.reset();
      ...
      if (redb_config.has_key("build_algo")) {
        ...
        if (algo_str == algo_label(BUILD_ALGO_STANDARD)) {
          ...
          DT_THROW(std::logic_error, "Invalid build algo label '" << algo_str << "'!");
        }
      }
      ...
      if (redb_config.has_key("rtd_buffer_capacity")) {
        cfg_.rtd_buffer_capacity = redb_config.fetch_positive_integer("rtd_buffer_capacity");
      }
    ...
    // static
    void builder_config::print_skel_config(std::ostream & out_, const int run_id_)
    {
      int run_id = run_id_;
      if (run_id < 100) {
      ...
      out_ <<
        ...
        "  \"snemo_run-" << run_id << "_red_part-0.data.gz\" \\          \n"
        "  \"snemo_run-" << run_id << "_red_part-1.data.gz\" \\          \n"
        "  \"snemo_run-" << run_id << "_red_part-2.data.gz\"             \n"
        "                                                                \n"
        "#@description Maximum number of RED records per output files (optional)\n"
      ...
      out_ <<
        "###########################################################\n";
      out_ <<
        "#@description Capacity of the buffer for RTD records (optional)   \n"
        "rtd_buffer_capacity    : integer = " << DEFAULT_RTD_BUFFER_CAPACITY << "\n"
        "                                                                       \n";
      out_ <<
        "# end.";
       ;
