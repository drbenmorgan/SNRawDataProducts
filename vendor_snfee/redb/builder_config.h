...
namespace snfee {
  namespace redb {
    ...
    public:
      ...
      {
        std::string              label;                        ///< Identification (human friendly string)
        std::vector<std::string> filenames;                    ///< Explicit list of output RED files
        std::size_t              max_records_per_file = 0;     ///< Maximum number of RED records per output file
        std::size_t              max_total_records    = 0;     ///< Maximum total number of RED records
        bool                     terminate_on_overrun = false; ///< Flag to silently terminate the overrunning writer (don't throw if set, unused)
        format_type              format = FORMAT_UNDEF;        ///< Format description (unused)
      };
      ...
      static void load(const std::string & config_filename_, builder_config & cfg_);
    ...
    public:
      ...
      uint32_t rtd_buffer_capacity = DEFAULT_RTD_BUFFER_CAPACITY; ///< Input RTD buffer capacity
