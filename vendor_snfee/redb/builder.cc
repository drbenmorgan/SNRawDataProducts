...
namespace snfee {
  namespace redb {
    ...
    void builder::initialize()
    {
      ...
      DT_THROW_IF(_config_.input_config.filenames.size() == 0
                  and _config_.input_config.listname.empty(),
                  std::logic_error,
    ...
    void builder::terminate()
    {
      ...
      DT_THROW_IF(!is_initialized(),
                  std::logic_error,
                  "RED builder is not initialized!");
      _initialized_ = false;
      _at_terminate_();
      DT_LOG_TRACE_EXITING(_logging_);
      return;
    ...
    void builder::_at_init_()
    {
      ...
      pimpl.builder = std::make_shared<rtd2red_builder>(*_pimpl_,
                                                        _config_.run_id,
                                                        _config_.build_algo,
                                                        _logging_);
    ...
    void builder::_at_terminate_()
    {
      if (_pimpl_) {
        ...
        if (pimpl.iworker) {
          ...
          iwResults.processed_records_counter1 = pimpl.iworker->get_records_counter();
