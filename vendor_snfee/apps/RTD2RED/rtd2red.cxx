  ...
  try {
    ...
    if (app_params.run_id != snfee::data::INVALID_RUN_ID) {
      if (redBuilderCfg.run_id != snfee::data::INVALID_RUN_ID) {
        DT_LOG_WARNING(app_params.logging, "Override run ID " << redBuilderCfg.run_id << " from the configuration file.");
    ...
    if (app_params.max_total_records != 0) {
      if (redBuilderCfg.output_config.max_total_records != 0) {
        DT_LOG_WARNING(app_params.logging, "Override max number of RED records "
                       << redBuilderCfg.output_config.max_total_records << " from the configuration file.");
        DT_LOG_WARNING(app_params.logging, "Max number of RED records : " << app_params.max_total_records);
     }
      redBuilderCfg.output_config.max_total_records = app_params.max_total_records;
    }

    if (app_params.rtd_buffer_capacity != 0) {
      if (redBuilderCfg.rtd_buffer_capacity != 0) {
        DT_LOG_WARNING(app_params.logging, "Override RTD input buffer capacity "
                       << redBuilderCfg.rtd_buffer_capacity << " from the configuration file.");
        DT_LOG_WARNING(app_params.logging, "RTD input buffer capacity is now : "
    ...
    /*
    {
      ...
      if (!app_params.report_filename.empty()) {
        ...
        redBuilderResultsOutfile.reset(new std::ofstream(redBuilderResultsFilename.c_str()));
      ...
      for (const auto & res : redBuilderResults) {
        *redBuilderResultsOut << "- Worker #" << i;
        if (res.category == snfee::redb::builder::WORKER_INPUT_RHD) {
          ...
          *redBuilderResultsOut << ",crate=" << snfee::model::crate_model_label(res.crate_model);
          *redBuilderResultsOut << ")"; 
        } else if (res.category == snfee::redb::builder::WORKER_OUTPUT_RED) {
          *redBuilderResultsOut << " (output RED)";
        } 
        *redBuilderResultsOut << " : " << std::endl;
        *redBuilderResultsOut << "   - Processed records : " << res.processed_records_counter1;
        *redBuilderResultsOut << std::endl;
        if (res.category == snfee::redb::builder::WORKER_OUTPUT_RED) {
          *redBuilderResultsOut << "   - Stored records    : " << res.processed_records_counter2;
          *redBuilderResultsOut << std::endl;
        }
        i++;
      }
