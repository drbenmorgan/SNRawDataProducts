...
struct app_params_type
{
  ...
  reader_config_type reader_config;
  ...
  try {
    ...
    opts.add_options()
      ...
      ("input-list,I",
       ...
       ->value_name("path"),
       "set the filename of the CBD input files list")
 
      ("input-file,i",
       po::value<std::vector<std::string>>(&app_params.input_filenames)
       ->value_name("path"),
       "add an CBD input filename")
      ...
      ("force-fake-trigger-ids,F",
       ...
       ->zero_tokens()
       ->default_value(false),
       "force trigger IDs and ignore those from CBD input files (expert)")
      ...
      ("max-cbd-per-input-file,C",
       po::value<std::size_t>(& app_params.max_cbd_per_input_file)
       ->value_name("number")
       ->default_value(0),
       "set the maximum number of collected CBD records per input file (expert)")
    ...
    if (vm.count("help")) {
      std::cout << "snfee-cbd2rhd : "
                << "Convert commissioning raw native binary data file (CBD) to official raw hit data file (RHD)"
                << std::endl << std::endl;
      std::cout << "Usage : " << std::endl << std::endl;
      std::cout << "  snfee-cbd2rhd [OPTIONS]" << std::endl << std::endl;  
      std::cout << opts << std::endl;
      std::cout << "Example : " << std::endl << std::endl;
      std::cout << " 1) Convert an unique CBD file to an unique RHD file : " << std::endl << std::endl;
      std::cout << "    snfee-cbd2rhd \\\n";
      std::cout << "      --crate-number 0 \\\n";
      std::cout << "      --input-file \"/data/SuperNEMO/CBD/Run_1000/calo/snemo_run-1000.bin\" \\\n";
      std::cout << "      --output-file \"snemo_run-1000_rhd_crate-0_part-0.xml\"";
      std::cout << std::endl << std::endl;  
      std::cout << " 2) Convert a list of CBD files to a list of RHD files: " << std::endl << std::endl;
      std::cout << "    snfee-cbd2rhd \\\n";
      std::cout << "      --crate-number 0 \\\n";
      std::cout << "      --force-fake-trigger-ids \\\n";
      std::cout << "      --max-records-per-file 500000 \\\n";
      std::cout << "      --dynamic-output-files \\\n";
      std::cout << "      --input-list \"snemo_run-1000_cbd_files.lis\" \\\n";
      std::cout << "      --output-file \"snemo_run-1000_rhd_crate-0.xml\" \n";
      std::cout << std::endl << std::endl;  
      return (-1);
    }
    ...
    std::string fake_trigger_id_save
      = app_params.work_dir + "/snfee-cbd2rhd_fake_trigger_ids-" + std::to_string(app_params.session_id) + ".save";
    ...
    DT_THROW_IF(app_params.input_filenames.empty(),
                std::logic_error, "Missing CBD input files!");
    std::size_t stored_rhd_counter = 0;
    std::size_t cbd_counter = 0;
    // Input file loop:
    ...
    for (int i_input_filename = 0;
      ...
      DT_LOG_INFORMATION(datatools::logger::PRIO_INFORMATION,
                         "Reading CBD input file : '" + app_params.reader_config.input_filename + "'");
      // Reader for CBD data file (for the commissioning phase using the SN CRATE SOFTWARE acquisition program):
      std::string datafile_path = app_params.reader_config.input_filename;
      datatools::fetch_path_with_env(datafile_path);
      // The raw native binary data reader:
      sncomdaq_run_reader reader;
      sncomdaq_run_reader_init(&reader,
                               datafile_path.c_str(),
                               datatools::logger::is_debug(app_params.reader_logging) ? 1 : 0);  
      sncomdaq_run_reader_print(&reader, stderr);
      std::size_t cbd_counter_for_this_file = 0;
      bool end_of_input_for_this_file = false;
      // Reader loop:
      while (sncomdaq_run_reader_has_next(&reader)) {
        DT_LOG_DEBUG(app_params.logging, "Loading next record...");

        sncomdaq_hit_type next_hit_category = sncomdaq_run_reader_next(&reader);
        if (next_hit_category == SNCOMDAQ_HIT_CALORIMETER) {
          DT_LOG_DEBUG(app_params.logging, "Found a calo hit record.");
          sncomdaq_calorimeter_hit caloHit;
          sncomdaq_run_reader_load_calorimeter_hit(&reader, &caloHit);

          snfee::data::calo_hit_record myCaloRec;
          cbd2rhd::convert(caloHit, myCaloRec);
        ...
        } else if (next_hit_category == SNCOMDAQ_HIT_TRACKER) {
          DT_LOG_DEBUG(app_params.logging, "Found a tracker hit record.");
          sncomdaq_tracker_hit trkHit;
          sncomdaq_run_reader_load_tracker_hit(&reader, &trkHit);

          snfee::data::tracker_hit_record myTrackerRec;
          cbd2rhd::convert(trkHit, myTrackerRec);
        ...
        } else if (next_hit_category == SNCOMDAQ_HIT_TRIGGER) {
          DT_LOG_WARNING(app_params.logging, "Found a trigger record.");
          sncomdaq_trigger_hit trigHit;
          sncomdaq_run_reader_load_trigger_hit(&reader, &trigHit);
          
          snfee::data::trigger_record myTrigRec;
          cbd2rhd::convert(trigHit, myTrigRec);
        ...
        cbd_counter++;
        cbd_counter_for_this_file++;
        // End of loop:
        if (cbd_counter % app_params.cbd_counter_period == 0) {
          DT_LOG_INFORMATION(datatools::logger::PRIO_INFORMATION, "Loaded CBD records: " << cbd_counter);
        }
        if (app_params.writer_config.max_total_records
            and cbd_counter == app_params.writer_config.max_total_records) {
          DT_LOG_INFORMATION(datatools::logger::PRIO_INFORMATION, "Max CBD records reached!");
          end_of_input_for_this_file = true;
          end_of_input = true;
        }
        if (app_params.max_cbd_per_input_file
            and cbd_counter_for_this_file == app_params.max_cbd_per_input_file) {
          DT_LOG_INFORMATION(datatools::logger::PRIO_INFORMATION, "Max CBD records per input file reached!");
          end_of_input_for_this_file = true;
        }
      ...
      sncomdaq_run_reader_terminate(&reader);  
      if (end_of_input) break;
