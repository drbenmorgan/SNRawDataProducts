...
namespace snfee {
  namespace rtdb {
    ...
    datatools::logger::priority builder::get_logging() const
    {
      return _logging_;
    }
    ...
    bool builder::is_initialized() const
    {
      return _initialized_;
    }

    void builder::set_config(const builder_config & cfg_)
    {
    ...
    void builder::terminate()
    {
      ...
      DT_THROW_IF(!is_initialized(),
                  std::logic_error,
                  "RDT builder is not initialized!");
      _initialized_ = false;
      _at_terminate_();
      DT_LOG_TRACE_EXITING(_logging_);
      return;
    ...
    /// current_trig_id      = 4
    /// rhd_recs             = [TRIGID=4][TRIGID=4][TRIGID=4][TRIGID=5][TRIGID=5][TRIGID=7][TRIGID=8][TRIGID=8]
    /// trig_ids             = {4,5,7,8}
    /// terminated           = false
    /// \endcode
    ///
    struct rhd_buffer
    {
      rhd_buffer(const int32_t id_, const uint32_t capacity_ = 0, const std::size_t min_popping_trig_ids_ = 2)
      {
        _id_ = id_;
          ...
          if (trigger_id_less(new_trig_id, _front_trig_id_)) {
           // Unsorted case:
            ...
            DT_THROW(std::logic_error,
                     "Unsorted input RHD: new trigger ID=" << new_trig_id << "<"
      ...
      {
        snfee::data::trigger_id_comparator_type trigger_id_less;
        // DT_LOG_DEBUG(_logging_, "About to insert record in RHD buffer " << _id_);
        int32_t new_trig_id = rhd_rec_.get_trigger_id();
        // DT_LOG_DEBUG(_logging_, "Buffer id=" << _id_ << " new trigger ID = " << new_trig_id);
        // if (_front_trig_id_ == snfee::data::INVALID_TRIGGER_ID) {
        //   _front_trig_id_ = new_trig_id;
        //   // DT_LOG_DEBUG(_logging_, "Buffer id=" << _id_ << " front trigger ID = " << _front_trig_id_);
        // }
        // DT_LOG_DEBUG(_logging_, "Buffer id=" << _id_ << " insert new trigger ID = " << new_trig_id);
        // DT_LOG_DEBUG(_logging_, "Buffer id=" << _id_ << " insert RHD record");
        // if (_trig_ids_.size()) {
        //   std::ostringstream outs;
        //   outs << "Input buffer # " << _id_ << " : " << std::endl;
        //   outs << "  New trigger ID   = " << rhd_rec_.get_trigger_id() << std::endl;
        //   outs << "  Front trigger ID = " << _front_trig_id_ << std::endl;
        //   outs << "  Min trigger ID   = " << *_trig_ids_.begin() << std::endl;
        //   outs << "  Max trigger ID   = " << *_trig_ids_.rbegin() << std::endl;
        //   std::cerr << outs.str();
        // }
        if (_rhd_recs_.size() == 0) {
          // RHD records =      [ ]
      ...
      */
      ...
      snfee::io::rhd_record pop_record()
      {
        DT_THROW_IF(is_empty(), std::logic_error, "No more record in the RHD buffer!");
      ...
      {
        if (is_empty()) return snfee::data::INVALID_TRIGGER_ID;
        if (is_terminated()) return _front_trig_id_;
      ...
      void print(std::ostream & out_) const
      {
        ...
        out << "|-- Minimum popping trig. IDs : " << _min_popping_trig_ids_ << std::endl;
        ...
        out << "|-- Embedded trigger IDs : " << _trig_ids_.size() << std::endl;
        {
          ...
          for (const auto & trigid : _trig_ids_) {
            bool print_it = false;
            if (counter == 0) print_it = true;
            if (counter + 1 == _trig_ids_.size()) print_it = true;
            if (print_it) {
              out << "|   ";
    ...
    struct input_worker
    {
      ...
      void run()
      {
        ...
        DT_LOG_DEBUG(_logging_, "Starting main running loop from worker [" << _id_ << "]...");
        ...
        while (!_stop_request_) {
          ...
          if (rec.empty()) {
            ...
            if (_preader_->has_record_tag()) {
              if (_preader_->record_tag_is(snfee::data::calo_hit_record::SERIAL_TAG)) {
                DT_LOG_DEBUG(_logging_, "Loading a new calo hit record from worker [" << _id_ << "]...");
                rec.make_calo_hit();
                _preader_->load(*rec.get_calo_hit_rec());
                DT_LOG_DEBUG(_logging_, "New calo hit record is loaded from worker [" << _id_ << "]...");
                _records_counter_++;
                DT_LOG_DEBUG(_logging_, "Input buffer #" << _id_ << " is unlocked by input worker.");
              } else if (_preader_->record_tag_is(snfee::data::tracker_hit_record::SERIAL_TAG)) {
                DT_LOG_DEBUG(_logging_, "Loading a new tracker hit record from worker [" << _id_ << "]...");
                rec.make_tracker_hit();
                _preader_->load(*rec.get_tracker_hit_rec());
                DT_LOG_DEBUG(_logging_, "New tracker hit record is loaded from worker [" << _id_ << "]...");
                _records_counter_++;
                DT_LOG_DEBUG(_logging_, "Input buffer #" << _id_ << " is unlocked by input worker.");
              } else if (_preader_->record_tag_is(snfee::data::trigger_record::SERIAL_TAG)) {
                DT_LOG_DEBUG(_logging_, "Loading a new trigger record from worker [" << _id_ << "]...");
                rec.make_trig();
                _preader_->load(*rec.get_trig_rec());
                DT_LOG_DEBUG(_logging_, "New trigger record is loaded from worker [" << _id_ << "]...");
                _records_counter_++;
                DT_LOG_DEBUG(_logging_, "Input buffer #" << _id_ << " is unlocked by input worker.");
              } else {
                DT_THROW(std::logic_error,
      ...
      std::mutex & _mtx_;     ///< Handle to the mutex associated to the input RHD buffer
      rhd_buffer & _buf_;     ///< Handle to the input RHD buffer
      bool _accept_unsorted_input_ = false;
      std::shared_ptr<snfee::io::multifile_data_reader> _preader_; ///< Data reader
    ...
    struct rtd_buffer
    {
      ...
      snfee::io::rtd_record pop_record()
      {
        ...
        _front_trigger_id_ = snfee::data::INVALID_TRIGGER_ID;
        if (_rtd_recs_.size()) {
          // Update the current trigger ID with the ones from the next record in the FIFO:
    ...
    {
      std::vector<input_worker_ptr> iworkers; ///< Collection of RHD input workers
      std::vector<rhd_buffer>       ibuffers; ///< Collection of buffers of input raw hit data records (RHD)
      std::vector<std::shared_ptr<std::mutex>> imtxs; ///< Collection of mutexes for individual access to input buffers
      std::shared_ptr<std::mutex>   gimtx;    ///< Mutex for access to all input buffers
    ...
    struct rhd2rtd_merger
    {
      ...
      std::size_t get_rtd_records_counter() const
      {
        return _rtd_records_counter_;
      };

      int32_t get_minimum_trigger_id_from_input_buffers() const
      {
        snfee::data::trigger_id_comparator_type trigger_id_less;
        ...
        for (int i = 0; i < (int) _pimpl_.ibuffers.size(); i++) {
          auto & ibuf = _pimpl_.ibuffers[i];
          {
            ...
            // DT_LOG_DEBUG(_logging_, "Input buffer #" << i << " is locked by merger.");
            if (!ibuf.is_finished()) {
              ...
              if (!cannot_determine) {
                if (ibuf.get_front_trig_id() != snfee::data::INVALID_TRIGGER_ID) {
                  if (mintrigid == snfee::data::INVALID_TRIGGER_ID
                      or trigger_id_less(next_trig_id, mintrigid)) {
                    mintrigid = next_trig_id;
                  }
                }
              }
            }
            // DT_LOG_DEBUG(_logging_, "Input buffer #" << i << " is unlocked by merger.");
          }
          if (cannot_determine) {
      ...
      { 
        ...
        while (!_stop_request_) {
          ...
          if (fetchable_trig_id == snfee::data::INVALID_TRIGGER_ID) {
            ...
            // DT_LOG_DEBUG(_logging_, "No fetchable trigger ID is available yet!");
          } else {
            // DT_LOG_DEBUG(_logging_, "Fetchable trigger ID is : " << fetchable_trig_id);
            process_input_rhd = true;
          }

          // Manage the completeness of the current RTD for pushing in the output buffer:
          bool push_current_rtd = false;
          if (process_input_rhd) {
            if (rtd_rec.get_trigger_id() != snfee::data::INVALID_TRIGGER_ID
                and trigger_id_less(rtd_rec.get_trigger_id(), fetchable_trig_id)) {
              // We already have a working RTD record and it has been completed
              // (no more records with same trigger ID are expected from the input buffers),
              // so we push it in the output buffer:
              DT_LOG_DEBUG(_logging_, "Make the current RTD pushable because input buffers cannot provide RHD records with trigger ID = " << rtd_rec.get_trigger_id() << " anymore.");
              push_current_rtd = true;
            }
          } else if (all_input_buffers_finished) {
            DT_LOG_DEBUG(_logging_, "Make the current RTD pushable because this is the end of the input buffers.");
            push_current_rtd = true;
          }
          ...
          if (push_current_rtd) {
                         ...
                         << rtd_rec.get_trigger_id());
            {
              ...
              DT_THROW_IF(_force_complete_rtd_ and !rtd_rec.get_rtd().is_complete(),
                          std::logic_error,
                          "Incomplete RTD data!");
              if (!rtd_rec.get_rtd().has_trig()) {
                if (_use_mock_trig_) {
                  auto & rtd = rtd_rec.grab_rtd();
                  auto trig_ptr = std::make_shared<snfee::data::trigger_record>();
                  int32_t trigger_hit_num = 0;
                  int32_t trigger_id = rtd_rec.get_trigger_id();
                  snfee::data::trigger_record::trigger_mode_type trigger_mode = snfee::data::trigger_record::TRIGGER_MODE_CALO_ONLY;
                  trig_ptr->make(trigger_hit_num,
                                 trigger_id,
                                 trigger_mode);
          ...
          if (process_input_rhd) {
            ...
            if (rtd_rec.get_trigger_id() == snfee::data::INVALID_TRIGGER_ID) {
              // We initiate a new RTD working record to host upcoming RHD records:
              DT_LOG_DEBUG(_logging_, "Make a new working RTD record with trigger ID = "
                           << fetchable_trig_id);
            ...
            for (int i = 0; i < (int) _pimpl_.ibuffers.size(); i++) {
              DT_LOG_DEBUG(_logging_, "Inspect input buffer #" << i);
              {
                ...
                // Extract all RHD records matching the working trigger ID from the input buffer:
                while (!ibuf.is_empty()) {
                  if (ibuf.get_front_trig_id() != rtd_rec.get_trigger_id()) {
                    break;
                  }
                  DT_LOG_DEBUG(_logging_, "Pop record from input buffer #" << i);
                  const snfee::io::rhd_record & rec = ibuf.pop_record();
                  if (datatools::logger::is_debug(_logging_)) {
                    rec.print(std::cerr);
                  }
                  DT_LOG_DEBUG(_logging_, "Install the RHD record from input buffer #"
                               << i << " into the RTD record...");
                  rtd_rec.install_rhd(rec);
                  DT_LOG_DEBUG(_logging_, "RHD record has been inserted in the current RTD record!");
                }
                if (datatools::logger::is_debug(_logging_)) {
                  ibuf.print(std::cerr);
                }
          ...
          if (!all_input_buffers_finished) {
            ...
            for (int i = 0; i < (int) _pimpl_.ibuffers.size(); i++) {
              ...
              if (ibuf.is_finished()) {
                ...
                if (!finished_buffers.count(i)) {
                  ...
                  // print_bufs = true;
                  DT_LOG_NOTICE(_logging_, "Input buffer #" << i << " is finished.");
                }
              }
              DT_LOG_DEBUG(_logging_, "Input buffer #" << i << " is unlocked by merger.");
            }
            ...
            //   // Devel:
            //   DT_LOG_DEBUG(_logging_, "# finished input buffers = " << nfinished << "/" << nin);
            //   DT_LOG_DEBUG(_logging_, "Status of input buffers: ");
            //   for (int i = 0; i < _pimpl_.ibuffers.size(); i++) {
            //     std::lock_guard<std::mutex> lck(*_pimpl_.imtxs[i]);
            //     const auto & ibuf = _pimpl_.ibuffers[i];
            //     ibuf.print(std::cerr);
            //   }
          ...
          if (all_input_buffers_finished
            ...
            DT_LOG_NOTICE(_logging_, "All input buffers are finished and there is no more current RTD record. Stop!");
            stop();
          }
        ...
        while (!_stop_request_) {
          {
            ...
            while (!_buf_.is_empty()) {
              DT_LOG_DEBUG(_logging_, "Pop RTD record from the output RTD buffer...");
              _records_counter_++;
              snfee::io::rtd_record rec = _buf_.pop_record();
              if (!writer_is_terminated and _pwriter_->is_terminated()) {
                writer_is_terminated = true;
                DT_LOG_NOTICE(_logging_, "Output RTD writer is now terminated.");
              }
              if (!writer_is_terminated) {
                ...
                DT_LOG_NOTICE(_logging_, "Output RTD writer: anticipated stop.");
                stop();
              }
            }
    ...
    void builder::_at_init_()
    {
      ...
      pimpl.oworker = std::make_shared<output_worker>(*pimpl.omtx.get(),
                                                      pimpl.obuffer,
      ...
      // Input managers:
      {
        ...
        for (const auto & iconfig : _config_.input_configs) {
          DT_LOG_NOTICE(_logging_, "Instantiating the input buffer mutex #" << icount << "...");
          pimpl.imtxs.push_back(std::make_shared<std::mutex>());
          DT_LOG_NOTICE(_logging_, "Instantiating the input buffer #" << icount << " for crate '"
                        << snfee::model::crate_model_label(iconfig.crate_model) << "'...");
          std::size_t min_popping_trig_ids = 1;
          if (_config_.accept_unsorted_records) {
            min_popping_trig_ids = _config_.unsorted_records_min_popping_safety_depth;
          }
          uint32_t capacity = 0;
      ...
      // Input workers:
      {
        int icount = 0;
        for (const auto & iconfig : _config_.input_configs) {
          DT_LOG_NOTICE(_logging_, "Instantiating the input worker #" << icount << "...");
          ...
          DT_LOG_DEBUG(_logging_, "Done for input worker #" << icount
      ...
      if (_pimpl_) {
        ...
        if (pimpl.iworkers.size()) {
          for (auto & iwkr : pimpl.iworkers) {
            ...
            DT_LOG_DEBUG(_logging_, "Destroying the input worker #" << icount << "...");
            iwkr.reset();
            icount++;
          }
    ...
    void builder::_at_run_()
    {
      ...
      for (std::size_t i = 0; i < _config_.input_configs.size(); i++) {
        DT_LOG_DEBUG(_logging_, "Starting thread for input worker #" << i << " at [@" << pimpl.iworkers[i].get() << "]...");
