...
namespace snfee {
  namespace model {
    ...
    std::string
    subdetector_label(const subdetector_type subdetector_)
    {
      switch (subdetector_) {
    ...
    std::string
    access_label(const access_type access_)
    {
      switch (access_) {
        case ACCESS_READ:
    ...
    std::string
    usage_label(const usage_type usage_)
    {
      switch (usage_) {
        case USAGE_EXPERT:
          return std::string("expert");
    ...
    std::string
    action_label(const action_type action_)
    {
      switch (action_) {
        case ACTION_CONFIGURATION:
          return std::string("configuration");
        case ACTION_DYNCONFIG:
          return std::string("dynconfig");
        case ACTION_MEASUREMENT:
          return std::string("measurement");
        case ACTION_DIAGNOSIS:
          return std::string("diagnosis");
        default:
          return none_label();
      }
    }
    ...
    std::string
    crate_model_label(const crate_model_type model_)
    {
      switch (model_) {
    ...
    std::string
    board_model_label(const board_model_type model_)
    {
      switch (model_) {
        case BOARD_TB:
        ...
        case BOARD_CFEB:
          return std::string("CFEB");
        case BOARD_TFEB:
          return std::string("TFEB");
        default:
          return none_label();
      }
    }
    ...
    std::string
    firmware_category_label(const firmware_category_type category_)
    {
      switch (category_) {
        case FWCAT_TB:
          return std::string("TB");
        case FWCAT_CB_CTRL:
          return std::string("CB_CTRL");
        case FWCAT_CB_FE:
          return std::string("CB_FE");
        case FWCAT_CFEB_CTRL:
          return std::string("CFEB_CTRL");
        case FWCAT_CFEB_FE:
          return std::string("CFEB_FE");
        case FWCAT_TFEB_CTRL:
          return std::string("TFEB_CTRL");
        case FWCAT_TFEB_FE:
          return std::string("TFEB_FE");
        default:
          return none_label();
      }
    }
    ...
    std::string
    fpga_model_label(const fpga_model_type model_)
    {
      switch (model_) {
        case FPGA_CONTROL:
          return "Control";
    ...
    bool
    parse(const std::string& word_, unsigned long& value_, int base_)
    {
    ...
    std::ostream&
    path_to_stream(std::ostream& out_, const path_type& p_)
    {
    ...
    std::string
    path_to_string(const path_type& path_)
    {
    ...
    path_type
    string_to_path(const std::string& token_)
    {
      path_type p;
    ...
    void
    truncate_path(const path_type& path_,
    ...
    path_type
    truncate_path(const path_type& path_, const std::size_t n_)
    {
      path_type tp;
