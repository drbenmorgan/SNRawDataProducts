...
namespace snfee {
  namespace model {
    ...
    enum action_type {
      ...
      ACTION_CONFIGURATION = 1, ///< Static configuration action (at run start configuration, writen only once)
      ACTION_DYNCONFIG     = 2, ///< Dynamic configuration action (on-the-fly configuration, can be changed during a run)
      ACTION_MEASUREMENT   = 3, ///< Measurement action (read production data, check/monitor configuration parameters...)
      ACTION_DIAGNOSIS     = 4  ///< Diagnosis action (read internal/expert status...)
    };
    ...
    /// \brief Supported categories of firmware, depending on the target FPGA and board.
    ...
    void truncate_path(const path_type & path_, path_type & trunc_path_, const std::size_t n_ = 1);
