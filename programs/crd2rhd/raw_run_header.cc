...
namespace snfee {
  namespace io {
    ...
    bool raw_run_header::is_complete() const
    {
    ...
    void raw_run_header::set_software_version(const std::string & sw_version_)
    {
      ...
      DT_THROW_IF(!ver_id.has_major(), std::logic_error,
                  "Missing major software version number in '" << ver << "'!");
      DT_THROW_IF(!ver_id.has_minor(), std::logic_error,
    ...
    void raw_run_header::make(const std::string & sw_version_,
    ...
    void raw_run_header::reset()
    {
      _reset_();
      return;
    }

    void raw_run_header::_reset_()
    {
      return;
    }

    // virtual
    void raw_run_header::print_tree(std::ostream & out_,
                                    const boost::property_tree::ptree & options_) const
    {
      ...
      out_ << popts.indent << tag
           << "Unix time              : [" << std::setprecision(15) << unix_time << " s]" << std::endl;
      ...
      out_ << popts.indent << inherit_tag(popts.inherit)
           << "Completeness           : " << std::boolalpha << is_complete() << std::endl;
