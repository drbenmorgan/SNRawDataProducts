...
namespace snfee {
  namespace io {
    ...
    raw_hit_reader::raw_hit_reader()
    {
      return;
    }

    bool raw_hit_reader::has_next_hit() const
    {
    ...
    bool raw_hit_reader::has_run_header() const
    {
      return _header_.get() != nullptr && _header_->is_complete();
    }

    void raw_hit_reader::load_run_header(raw_run_header & header_)
    {
      DT_THROW_IF(!_initialized_, std::logic_error, "Reader is not initialized!");
    ...
    datatools::logger::priority raw_hit_reader::get_logging() const
    {
      return _logging_;
    }

    void raw_hit_reader::set_logging(datatools::logger::priority logging_)
    {
      _logging_ = logging_;
      return;
    }

    const raw_hit_reader::config_type & raw_hit_reader::get_config() const
    {
      return _config_;
    }

    void raw_hit_reader::set_config(const config_type & cfg_)
    {
    ...
    bool raw_hit_reader::is_initialized() const
    {
    ...
    void raw_hit_reader::print(std::ostream & out_) const
    {
      out_ << "Raw hit reader:" << std::endl;
      out_ << "|-- " << "Config : " << std::endl;
      out_ << "|   " << "|-- " << "Input filename : " << _config_.input_filename << std::endl;
      out_ << "|   " << "|-- " << "Crate number   : " << _config_.crate_num << std::endl;
      out_ << "|   " << "|-- " << "With calo   : " << std::boolalpha << _config_.with_calo << std::endl;
      out_ << "|   " << "|-- " << "With tracker   : " << std::boolalpha << _config_.with_tracker << std::endl;
      out_ << "|   " << "`-- " << "With calo waveforms : " << std::boolalpha << _config_.with_calo_waveforms << std::endl;
      out_ << "`-- " << "Initialized  : " << std::boolalpha << _initialized_ << std::endl;
      return;
    }

    void raw_hit_reader::_init_input_file_()
    {
      _fin_.reset(new std::ifstream);
      std::string ifn = _config_.input_filename;
      datatools::fetch_path_with_env(ifn);
      _fin_->open(ifn.c_str());
      DT_THROW_IF(!*_fin_, std::runtime_error, "Cannot open input file '" << _config_.input_filename << "'");
      return;
    }

    void raw_hit_reader::_reset_input_file_()
    {
      if (_fin_) {
        _fin_->close();
        _fin_.reset();
    ...
    void raw_hit_reader::_init_header_()
    {
    ...
    void raw_hit_reader::_reset_header_()
    {
      if (_header_) {
        _header_.reset();
      }
    ...
    void raw_hit_reader::_init_parser_()
    {
      ...
      parser_config.firmware_version
        = datatools::version_id(_header_->software_major_version,
                                _header_->software_minor_version);
    ...
    void raw_hit_reader::_decode_header_(const std::string & hline_, const int index_)
    {
