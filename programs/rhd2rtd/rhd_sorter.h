...
namespace snfee {
  namespace io {
    ...
    class rhd_sorter : private boost::noncopyable {
    public:
      ...
      struct config_type {
        std::string input_rhd_listname; ///< Name of a file with a sequence of
                                        ///< RHD input filenames
        std::vector<std::string>
          input_rhd_filenames;  ///< Sequence of RHD input filenames
        std::string mode_label; ///< Running mode ('sort' or 'evaluation')
        bool no_store = false;  ///< No store flag
        std::string output_rhd_listname; ///< Name of a file with a sequence of
                                         ///< sorted RHD output filenames
        std::vector<std::string>
          output_rhd_filenames; ///< Sequence of sorted RHD output filenames
        std::size_t max_records_per_file =
          0; ///< Max number of RHD records per file
        // std::size_t max_total_records = 0;          ///< Max number of RHD
        // records (very risky)
        std::size_t sort_buffer_size =
          DEFAULT_SORT_BUFFER_SIZE; ///< Size of the sorting buffer
        std::size_t evaluation_buffer_size =
          DEFAULT_EVALUATION_BUFFER_SIZE; ///< Size of the evaluation buffer
        std::size_t log_modulo = 10000;   ///< Modulo for log print
        std::string
          report_filename; ///< Name of a report file ('sort' or 'evaluation')
      };
