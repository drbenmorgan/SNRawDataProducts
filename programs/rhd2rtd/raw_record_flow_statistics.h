...
namespace snfee {
  namespace io {
    ...
    private:
      ...
      // Work:
      std::map<int, int>
        _first_positions_; ///< Buffer of record of trigger IDs' first positions
      std::map<int, inversion_record> _inversions_; ///< List of inversions
      int _max_inversion_distance_ =
        -1; ///< Maximum detected inversion distance
      snfee::tools::histogram_int
        _hdistances_; ///< Histogram of distances of inversion
    };
