...
namespace snfee {
  namespace rtdb {
    ...
    // static
    std::string builder_config::format_label(const format_type f_)
    {
    ...
    bool builder_config::is_force_complete_rtd() const
    {
      return force_complete_rtd;
    }

    void builder_config::set_force_complete_rtd(bool f_)
    {
    ...
    void builder_config::set_output_config(const std::string & label_,
    ...
    const builder_config::output_config_type & builder_config::get_output_config() const
    {
      return output_config;
    }

    void builder_config::add_input_config(const std::string & label_,
                                    ...
                                    const boost::property_tree::ptree & options_) const
    {
      ...
      outs << popts.indent << tag
           << "Input configs : " << input_configs.size() << std::endl;
      for (std::size_t i = 0; i < input_configs.size(); i++) {
        ...
        for (int ifilename = 0; ifilename < (int) ic.filenames.size(); ifilename++) {
      ...
      outs << popts.indent << skip_tag << tag
           << "Label : '" << output_config.label << "'" << std::endl;

      outs << popts.indent << skip_tag << tag
           << "Filenames : " << output_config.filenames.size() << std::endl;

      for (int ifilename = 0; ifilename < (int) output_config.filenames.size(); ifilename++) {
      ...
      outs << popts.indent << skip_tag << tag
           << "Max total records : " << output_config.max_total_records << std::endl;

      outs << popts.indent << skip_tag << tag
           << "Terminate/overrun : " << std::boolalpha << output_config.terminate_on_overrun << std::endl;

      outs << popts.indent << skip_tag << last_tag
           << "Format : '" << format_label(output_config.format) << "'" << std::endl;

      outs << popts.indent << skip_tag << last_tag
           << "Calo RHD buffer capacity : " << calo_rhd_buffer_capacity << std::endl;
    ...
    void builder_config::check(const builder_config & cfg_)
    {
      ...
      for (const auto & iconf : cfg_.input_configs) {
        ...
        DT_THROW_IF(iconf.filenames.size() == 0 and iconf.listname.empty(),
                    std::logic_error,
                    "Missing RHD input #" << icount << " filenames or listname!");
        icount++;
      }
                              ...
                              builder_config & cfg_)
    {
      ...
      for (const std::string & inputName : inputs) {
        ...
        }
        {
          ...
          if (rtdb_config.has_key(key)) {
            icfg.crate_model = snfee::model::crate_model_from(rtdb_config.fetch_string(key));
          }
        }
        {
    ...
    // static
    void builder_config::print_skel_config(std::ostream & out_,
                                           const int run_id_,
                                           const uint16_t nbcrates_)
    {
      ...
      for (int icrate = 0; icrate < (int) nbcrates_; icrate++) {
        ...
        std::string crate_model  = snfee::model::subdetector_label(snfee::model::SUBDETECTOR_CALORIMETER);
        ...
        out_ <<
          ...
          "                                                                        \n"
          "#@description Crate ID associated to the '" << crate_label << "' input (mandatory)\n"
          "rhd.inputs." << crate_label << ".crate_id : integer = " << crate_id << " \n"
          "                                                           \n"
          "#@description File with the list of RHD input files for the '" << crate_label << "' input   \n"
          "# This parameter is mandatory if the below parameter 'filenames' is not provided.\n"
          "# The list file uses the following format (one filename per line) :\n"
          "#   \n"
          "#   # List of RHD filenames: \n"
          "#   snemo_run-" << run_id << "_rhd_" << crate_prefix << "-" << crate_id << "_part-0.data.gz \n"
          "#   snemo_run-" << run_id << "_rhd_" << crate_prefix << "-" << crate_id << "_part-1.data.gz \n"
          "#   ... \n"
          "#   snemo_run-" << run_id << "_rhd_" << crate_prefix << "-" << crate_id << "_part-99.data.gz \n"
          "#   # End.\n"
          "#   \n"
          "rhd.inputs." << crate_label << ".listname : string as path = \"snemo_run-" << run_id << "_rhd_files.lis\" \n"
          "                                                                             \n"
          "# #@description Explicit list of RHD input files for the '" << crate_label << "' input \n"
          "# # This parameter is mandatory if the above parameter 'listname' is not provided.\n"
          "# rhd.inputs." << crate_label << ".filenames : string[3] as path = \\ \n"
          "#   \"snemo_run-" << run_id << "_rhd_" << crate_prefix << "-" << crate_id << "_part-0.data.gz\" \\    \n"
          "#   \"snemo_run-" << run_id << "_rhd_" << crate_prefix << "-" << crate_id << "_part-1.data.gz\" \\    \n"
          "#   \"snemo_run-" << run_id << "_rhd_" << crate_prefix << "-" << crate_id << "_part-2.data.gz\"       \n"
          "                                                    \n";
      }
      ...
      out_ <<
        ...
        "  \"snemo_run-" << run_id << "_rtd_part-2.data.gz\"             \n"
        "                                                                \n"
        "#@description Maximum number of RTD records per output files (optional)\n"
      ...
      out_ <<
        "###########################################################\n";
      out_ <<
        "#@description Capacity of the buffer for calo RHD records (optional)   \n"
        "calo_rhd_buffer_capacity    : integer = " << DEFAULT_CALO_RHD_BUFFER_CAPACITY << "\n"
        "                                                                       \n"
        "#@description Capacity of the buffer for tracker RHD records (optional)\n"
        "tracker_rhd_buffer_capacity : integer = " << DEFAULT_TRACKER_RHD_BUFFER_CAPACITY << "\n"
        "                                                                       \n";
      out_ <<
        "# end.";
       ;
    ...
    void builder_config::print_documentation(std::ostream & out_)
    {
      out_ <<
        ...
        "                                                              \n"
        "   #@description Crate model associated to the first RHD input (mandatory)\n"
        "   rhd.inputs.Calo0.crate_model : string = \"Calorimeter\"    \n"
        "                                                              \n"
        "   #@description Crate ID associated to the first RHD input (mandatory)\n"
        "   rhd.inputs.Calo0.crate_id : integer = 0                    \n"
        "                                                              \n"
        "   #@description File with the list of RHD input files for the first RHD input   \n"
        "   # This parameter is mandatory if the below parameter 'filenames' is not provided.\n"
        "   rhd.inputs.Calo0.listname : string as path = \"snemo_run-104_rhd_files.lis\" \n"
        "                                                                                \n"
        "   #@description Explicit list of RHD input files for the first RHD input \n"
        "   # This parameter is mandatory if the above parameter 'listname' is not provided.\n"
        ...
        "                                                       \n"
        "   #@description Crate model associated to the second RHD input (mandatory)      \n"
        "   rhd.inputs.Tracker0.crate_model : string = \"Tracker\"           \n"
        "                                                                    \n"
        "   #@description Crate ID associated to the second RHD input (mandatory)         \n"
        "   rhd.inputs.Tracker0.crate_id : integer = 0                       \n"
        "                                                                    \n"
        "   #@description File with the list of RHD input files for the second RHD input \n"
        "   # This parameter is mandatory if the below parameter 'filenames' is not provided.\n"
        "   rhd.inputs.Tracker0.listname : string as path = \"snemo_run-104_rhd_files.lis\" \n"
        "                                                                                   \n"
        "   #@description Explicit list of RHD input files for the second RHD input \n"
        "   # This parameter is mandatory if the above parameter 'listname' is not provided.\n"
