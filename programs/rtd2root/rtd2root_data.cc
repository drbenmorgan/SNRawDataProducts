...
namespace snfee {
  namespace data {
    ...
    // static
    void
    rtd2root_data::export_to_root(const raw_trigger_data& in_,
                                  rtd2root_data& out_)
    {
      out_.clear();
      ...
      for (const auto& hchit : in_.get_calo_hits()) {
        ...
        out_.calo_waveform_start_sample[calo_count] =
          chit.get_waveform_start_sample();
        out_.calo_waveform_number_of_samples[calo_count] =
          chit.get_waveform_number_of_samples();

        out_.calo_ch0_lt[calo_count] = chit.get_channel_data(0).is_lt();
        out_.calo_ch0_ht[calo_count] = chit.get_channel_data(0).is_ht();
        out_.calo_ch0_underflow[calo_count] =
          chit.get_channel_data(0).is_underflow();
        out_.calo_ch0_overflow[calo_count] =
          chit.get_channel_data(0).is_overflow();
        out_.calo_ch0_baseline[calo_count] =
          chit.get_channel_data(0).get_baseline();
        out_.calo_ch0_peak[calo_count] = chit.get_channel_data(0).get_peak();
        out_.calo_ch0_peak_cell[calo_count] =
          chit.get_channel_data(0).get_peak_cell();
        out_.calo_ch0_charge[calo_count] =
          chit.get_channel_data(0).get_charge();
        out_.calo_ch0_rising_cell[calo_count] =
        ...
        out_.calo_ch1_underflow[calo_count] =
          chit.get_channel_data(1).is_underflow();
        out_.calo_ch1_overflow[calo_count] =
          chit.get_channel_data(1).is_overflow();
        out_.calo_ch1_baseline[calo_count] =
          chit.get_channel_data(1).get_baseline();
        out_.calo_ch1_peak[calo_count] = chit.get_channel_data(1).get_peak();
        out_.calo_ch1_peak_cell[calo_count] =
          chit.get_channel_data(1).get_peak_cell();
        out_.calo_ch1_charge[calo_count] =
          chit.get_channel_data(1).get_charge();
        out_.calo_ch1_rising_cell[calo_count] =
        ...
        const snfee::data::calo_hit_record::waveforms_record& waveforms =
          chit.get_waveforms();
        for (int isample = 0; isample < chit.get_waveform_number_of_samples();
             isample++) {
          out_.calo_ch0_waveform[calo_count][isample] =
            waveforms.get_adc(isample, 0);
          out_.calo_ch1_waveform[calo_count][isample] =
            waveforms.get_adc(isample, 1);
        }
      ...
      for (const auto& hthit : in_.get_tracker_hits()) {
        ...
        out_.tracker_channel_category[tracker_count] =
          (int16_t)thit.get_channel_category();
        out_.tracker_timestamp_category[tracker_count] =
          (int16_t)thit.get_timestamp_category();
